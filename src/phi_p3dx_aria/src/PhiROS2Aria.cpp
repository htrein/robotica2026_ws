...
PhiROS2Aria::PhiROS2Aria() : Node("phi_aria_p3dx"),
                             ...
                             publish_aria_lasers(false), sonar_enabled(true), publish_sonar(true),
...
// TicksMM(-1), DriftFactor(-99999), RevCount(-1),
{
    ...
    sonar_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("sonar_cloud", 5);
    sonarScan_pub = this->create_publisher<sensor_msgs::msg::LaserScan>("sonar", 5);
    ...
    // Initialize sonar scan message
    sonarScanMsg_.header.frame_id = frame_id_sonar;
    sonarScanMsg_.angle_min = -M_PI;  // Full 360 degrees, but sonars are typically front-facing
    sonarScanMsg_.angle_max = M_PI;
    sonarScanMsg_.angle_increment = 2 * M_PI / 16;  // Assuming 16 sonars, adjust as needed
    sonarScanMsg_.range_min = 0.0;
    sonarScanMsg_.range_max = 5.0;  // Max range of sonars in meters
    sonarScanMsg_.scan_time = 0.1;  // Approximate scan time
...
void PhiROS2Aria::publish()
{
    ...
    if (publish_sonar)
    {
        ...
        // Create LaserScan message for sonar
        sonarScanMsg_.header.stamp = position.header.stamp;
        sonarScanMsg_.header.frame_id = frame_id_sonar;

        // Collect angles and ranges from each sonar sensor
        ...
        for (int i = 0; i < robot->getNumSonar(); i++)
        {
            ...
            // Get sensor position to calculate angle
            double sensor_x = reading->getSensorX() / 1000.0;
            double sensor_y = reading->getSensorY() / 1000.0;
            double angle = atan2(sensor_y, sensor_x);
            double range = reading->getRange() / 1000.0;

            angle_range_pairs.push_back(std::make_pair(angle, range));
        }
        ...
        // Fill LaserScan
        sonarScanMsg_.ranges.resize(angle_range_pairs.size());
        sonarScanMsg_.intensities.resize(angle_range_pairs.size());
        if (!angle_range_pairs.empty())
        {
            sonarScanMsg_.angle_min = angle_range_pairs.front().first;
            sonarScanMsg_.angle_max = angle_range_pairs.back().first;
            sonarScanMsg_.angle_increment = (angle_range_pairs.size() > 1) ?
                (sonarScanMsg_.angle_max - sonarScanMsg_.angle_min) / (angle_range_pairs.size() - 1) : 0.0;

            for (size_t i = 0; i < angle_range_pairs.size(); ++i)
            {
                sonarScanMsg_.ranges[i] = angle_range_pairs[i].second;
                sonarScanMsg_.intensities[i] = 1.0;
            }
        }

        // publish topics
        sonar_pub->publish(cloud);
        sonarScan_pub->publish(sonarScanMsg_);
